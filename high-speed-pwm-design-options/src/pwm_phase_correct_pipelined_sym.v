...
// SYMMETRIC dead-time variant of pwm_phase_correct_pipelined.v.
//
...
//
// Both halves use the same two thresholds; the only difference is which
// counter feeds the compare (counter_up for up half, counter_down for
// down half). Per phase the compare lane count is unchanged (4 lanes),
// but only TWO unique RHS values per phase instead of three.
//
// Total ON-time per period, gate_high : 2*duty - dt   (same as asymmetric)
// Total ON-time per period, gate_low  : 4096 - 2*duty - dt   (same)
// Dead-time width                     : dt cycles   (same)
// Phase shift vs asymmetric           : gate edges shift dt/2 cycles
//                                       inward; pulse center unchanged.
//
// Counter source: registered passthrough copies of addr (same as
// pwm_phase_correct_pipelined.v). Pipeline depth addr -> gate FF: 4
// stages. addr[11] pipelined through dir_d0..d2.
//
...
//
// Companion modules: pwm_phase_correct_twin_sym, pwm_phase_correct_brams_sym.

module pwm_phase_correct_pipelined_sym(
    input  wire        clk,
    input  wire        rst_n,
    ...
    // ============================================================
    // Bus-decoded duty thresholds (slow-domain writes pre-compute
    // duty ± dt/2, saturating).
    // ============================================================
    ...
    // ============================================================
    // Direction pipeline (3 explicit flops to match 4-stage data pipe).
    // ============================================================
    ...
    //   Up path:   counter_up   <  duty_u_minus_dt_half  -> lt_dmh_up
    //              counter_up   <  duty_u_plus_dt_half   -> lt_dph_up
    //   Down path: counter_down <  duty_u_minus_dt_half  -> lt_dmh_dn
    //              counter_down <  duty_u_plus_dt_half   -> lt_dph_dn
    // gate_uh = dir ? lt_dmh_dn : lt_dmh_up   (high  = counter<duty-dt/2)
    // gate_ul = dir ? ~lt_dph_dn : ~lt_dph_up (low  = counter>=duty+dt/2)
    // ============================================================
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            gate_uh <= dir_d2 ?  s2u_dn_lt_dmh :  s2u_up_lt_dmh;
            gate_ul <= dir_d2 ? ~s2u_dn_lt_dph : ~s2u_up_lt_dph;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            gate_vh <= dir_d2 ?  s2v_dn_lt_dmh :  s2v_up_lt_dmh;
            gate_vl <= dir_d2 ? ~s2v_dn_lt_dph : ~s2v_up_lt_dph;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            gate_wh <= dir_d2 ?  s2w_dn_lt_dmh :  s2w_up_lt_dmh;
            gate_wl <= dir_d2 ? ~s2w_dn_lt_dph : ~s2w_up_lt_dph;
        end
    end
